...
class TableColumn {
    constructor(period, table) {
        this.period = period;
        this.table = Object.freeze(table.map((entry) => {
    ...
    if (column !== undefined) {
        ...
        if (adjustment !== DamageRollDie.Full) {
            if (apPerDie === 5) {
                ...
                assert.that(extra !== 0, `Couldn't find adjustment ${adjustment} in table for ${apPerDie} AP per die`);
            }
        }
...
export function diceForDCs(dc, apPerDie) {
    assert.precondition(isKnownApPerDie(apPerDie), `unsupported AP per die ${apPerDie}`);
    let dice = 0;
    let adjustment = 0;
    if (apPerDie === 5) {
        dice = Math.floor(dc);
