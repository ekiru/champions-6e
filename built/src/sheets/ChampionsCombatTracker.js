    ...
    async getData() {
        ...
        if (context.hasCombat) {
            ...
            if (context.hasSpdChanges) {
                context.spdChangeTooltip = await renderTemplate("systems/champions-6e/templates/ui/combat-tracker/update-phases-tooltip.hbs", { changes: context.combat.pendingChanges });
            }
            const phases = context.combat.phaseChart;
            const startingPoint = context.combat.round === 1 ? 12 : 1;
            let i = 0;
            for (let segmentNumber = startingPoint; segmentNumber <= 12; segmentNumber++) {
                const segment = {
