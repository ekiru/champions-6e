...
import { countKillingDamage, countNormalDamage, Damage, } from "./mechanics/damage.js";
/**
...
const successRollTemplate = "systems/champions-6e/templates/dialog/success-roll.hbs";
/**
...
const canHitMessageTemplate = ({ label, dcv, dcvLabel }) => `${label} can hit ${dcvLabel} = ${dcv}`;
/**
...
const attackRollTemplate = "systems/champions-6e/templates/dialog/attack-roll.hbs";
/**
 ...
 */
export async function attackRollDialog(label, ocv, { actor, dcvLabel = "DCV", maneuverModifierLabel } = {}) {
...
export async function performKillingDamageRoll(dice, options = {}) {
    ...
    const response = countKillingDamage(rolledDice, multiplier, hasHalf && result.dice[2].total, dice.plusOrMinus);
    response.roll = result;
    await addMessage(formatDamage("Killing", options.label, diceString, response), response, foundry.utils.mergeObject(options, {
        messageFlags: {
            body: response.body,
            modifiers: +1,
            label: options.label,
...
const knockbackButtonHtml = '<button class="knockback-roll"><i class="fas fa-dice"></i> Roll Knockback</button>';
...
export async function performNormalDamageRoll(dice, options = {}) {
    ...
    const result = await roll.roll({ async: true });
    const rolledDice = result.dice[0].results.map((res) => res.result);
    const response = countNormalDamage(rolledDice, hasHalf && result.dice[1].total, dice.plusOrMinus);
    response.roll = result;
    await addMessage(formatDamage("Normal", options.label, diceString, response, options), response, foundry.utils.mergeObject(options, {
        messageFlags: { body: response.body, modifiers: 0, label: options.label },
    }));
    return response;
}
const damageRollTemplate = "systems/champions-6e/templates/dialog/damage-roll.hbs";
/**
 ...
 */
export async function damageRollDialog(label, dice, type, apPerDie, { actor, strDcs } = {}) {
    ...
    rollDialog(title, damageRollTemplate, context, (html) => {
        ...
        const diceAdjustment = Number(html.find("select[name='diceSuffix']").get(0).value);
        const addedDcs = Number(html.find("input[name='dcs']").get(0).value);
        const addedStr = strDcs && html.find("input[name='addStr']").get(0).checked ? strDcs : 0;
 ...
 */
export async function performKnockbackRoll({ body }, modifier = 0, options = {}) {
    const rollClass = options.Roll ?? Roll;
    const dice = modifier > -2 ? 2 + modifier : 0;
    const roll = body === 0 ? new rollClass("-1") : new rollClass(`${body} - ${dice}d6`);
    const result = await roll.roll({ async: true });
...
const knockbackRollTemplate = "systems/champions-6e/templates/dialog/knockback-roll.hbs";
/**
 ...
 * @param {Actor} options.actor The actor for whom the roll is being performed.
 */
export async function knockbackRollDialog(label, body, modifiers, { actor } = {}) {
