/* eslint-env jest */
import { EffectFlags, SystemActiveEffectModes, } from "../../../src/constants.js";
import { HALVED, Maneuver, NOT_APPLICABLE, SpecialModifier, standardManeuvers, TIME, } from "../../../src/mechanics/maneuvers.js";
...
describe("The Maneuver class", function () {
    describe("constructor", function () {
        ...
        it("should throw without an OCV modifier", function () {
            expect(() => new Maneuver("Strike", { time: 0.5, dcv: +0, summary })).toThrow("missing OCV modifier");
        });
        it("should throw without a DCV modifier", function () {
            expect(() => new Maneuver("Strike", { time: 0.5, ocv: +0, summary })).toThrow("missing DCV modifier");
        });
        it("should throw without a time", function () {
            expect(() => new Maneuver("Strike", { ocv: +0, dcv: +0, summary })).toThrow("missing time");
        });
        it("should throw without a summary", function () {
            expect(() => new Maneuver("Strike", { time: TIME.HALF_PHASE, ocv: +0, dcv: +0 })).toThrow("missing or invalid summary");
        });
    });
    describe("Maneuver.summarizeEffect", function () {
        const dcvTotal = "system.characteristics.dcv.total";
        it("should note DCV ±", function () {
            expect(Maneuver.summarizeEffect([
                {
                    key: dcvTotal,
                    value: +5,
    ...
    describe("isRolled", function () {
        it("should normally be true for most OCV modifiers", function () {
            const maneuver = (ocv) => new Maneuver("Strike", {
                ocv,
                dcv: +0,
                time: TIME.HALF_PHASE,
                summary: "",
    ...
    describe("calculateOcv", function () {
        const maneuver = (ocv) => new Maneuver("Kick", {
            ocv,
            dcv: +0,
            time: TIME.HALF_PHASE,
            summary: "A kick",
        ...
        it("should throw for a — modifier", function () {
            expect(() => maneuver(NOT_APPLICABLE).calculateOcv(5)).toThrow("OCV is not applicable to");
        });
        it("should pass the OCV through unchanged for special modifiers", function () {
            expect(maneuver(new SpecialModifier("randomize it", "random number")).calculateOcv(9)).toBe(9);
        });
    });
    describe("getEffectChanges", function () {
        const maneuver = (ocv, dcv) => new Maneuver("Flip Kick", {
            ocv,
            dcv,
            time: TIME.HALF_PHASE,
            summary: "A kick",
        ...
        it("should do no changes for now for special DCV modifiers", function () {
            expect(maneuver(+0, new SpecialModifier("var", "var")).getEffectChanges()).toHaveLength(0);
        });
...
describe("Combat Maneuvers", function () {
    ...
    describe("Brace", function () {
        const brace = standardManeuvers.find((maneuver) => maneuver.name === "Brace");
    ...
    describe("Set", function () {
        ...
        it("should return a summary with its effect", function () {
            ...
            expect(effect.flags["champions-6e"][EffectFlags.SUMMARY]).toContain("+1 to OCV");
        });
    });
});
