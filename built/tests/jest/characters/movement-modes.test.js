...
describe("MovementMode", function () {
    describe("constructor", function () {
        ...
        it("should require a string name", function () {
            expect(() => new MovementMode(5, { type, distance })).toThrow(new Error("name must be a string"));
        });
        it("should require a PowerType for a type", function () {
            expect(() => new MovementMode(name, { type: "Teleportation", distance })).toThrow(new Error("type must be a PowerType"));
        });
        it("should require a ModifiableValue distance", function () {
            expect(() => new MovementMode(name, { type, distance: 20 })).toThrow(new Error("distance must be a ModifiableValue"));
        });
    });
