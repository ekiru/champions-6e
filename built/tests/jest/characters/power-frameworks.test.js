// eslint-env jest
import { CustomPowerType, Power, StandardPowerType, } from "../../../src/mechanics/power.js";
import { Slot, SlotType, WarningScope, } from "../../../src/mechanics/powers/frameworks.js";
import { FrameworkModifier, FrameworkModifierScope, PowerLimitation, } from "../../../src/mechanics/powers/modifiers.js";
...
describe("Multipowers", function () {
    describe("new multipowers", function () {
        ...
        it("must have a name", function () {
            expect(() => new Multipower(5, { description, reserve })).toThrow(new Error("name must be a string"));
        });
        it("must have a description", function () {
            expect(() => new Multipower(name, { description: 5, reserve })).toThrow(new Error("description must be a string"));
        });
        it("must have an integral reserve", function () {
            expect(() => new Multipower(name, { description, reserve: "60" })).toThrow(new Error("reserve must be a non-negative integer"));
        });
        it("has no slots by default", function () {
            expect(new Multipower(name, { description, reserve })).toHaveProperty("slots", []);
        });
        ...
        it("has no modifiers by default", function () {
            expect(new Multipower(name, { description, reserve })).toHaveProperty("modifiers", []);
        });
        it("exposes any modifiers it was created with", function () {
            const focus = new FrameworkModifier(new PowerLimitation("OIF", {
                summary: "Obvious Inaccessible Focus",
                value: -0.5,
                id: "oif",
                description: "",
            }));
            expect(new Multipower(name, { description, reserve, modifiers: [focus] })).toHaveProperty("modifiers", [focus]);
        });
    });
    describe("fromItem", function () {
        ...
        it("should parse a valid multipower correctly", function () {
            ...
            expect(mp.modifiers[0]).toHaveProperty("scope", FrameworkModifierScope.SlotsOnly);
                ...
                slots: [
                    ...
                    new Slot({
                        power: new Power("Beguile", {
                            ...
                            description: "Enchant your listeners into perceiving what you wish",
                        }),
                        type: SlotType.Fixed,
                        fullCost: 60,
                        active: false,
                    ...
                    new Slot({
                        power: new Power("Charm", {
                            ...
                            summary: "+15 PRE",
                            description: "Subtly increase your charm with a charming lilt to your words.",
                        }),
                        active: true,
                        fullCost: 15,
                        type: SlotType.Fixed,
    ...
    describe("warnings", function () {
        ...
        it("should warn if a power is bigger than the reserve", function () {
            ...
            expect(mp.warnings[0]).toHaveProperty("message", "Slot active points are larger than the framework's reserve");
        ...
        it("should warn if more points are allocated than the reserve", function () {
            ...
            expect(mp.warnings).toHaveLength(1);
            expect(mp.warnings[0]).toHaveProperty("message", "More active points are allocated than fit in the framework's reserve");
        ...
        it("should warn if more points are allocated to a slot than the slot can use", function () {
            ...
            expect(mp.warnings).toHaveLength(1);
            expect(mp.warnings[0]).toHaveProperty("message", "This slot has more points allocated to it than it can use");
            expect(mp.warnings[0]).toHaveProperty("scope", WarningScope.Slot),
                expect(mp.warnings[0]).toHaveProperty("slotId", "chokingpollen");
        });
    });
...
describe("Variable Power Pools", function () {
    describe("new VPPs", function () {
        ...
        const modifiers = [];
        it("must have a name", function () {
            expect(() => new VPP(null, {})).toThrow(new Error("name must be a string"));
        });
        it("must have an integral pool", function () {
            expect(() => new VPP(name, { control, pool: "30", slots, modifiers, description })).toThrow(new Error("pool must be an integer"));
        });
        it("must have an integral control", function () {
            expect(() => new VPP(name, { control: "60", pool, slots, modifiers, description })).toThrow(new Error("control must be an integer"));
        });
        it("should expose its properties", function () {
            expect(new VPP(name, { control, pool, slots, modifiers, description })).toEqual({
                name,
                control,
                pool,
                slots,
    ...
    describe("fromItem", function () {
        ...
        it("should parse a valid VPP correctly", function () {
            ...
            expect(vpp).toHaveProperty("description", "<p>Flexible magical powers</p>");
    ...
    describe("warnings", function () {
        ...
        it("should warn if a power's AP exceeds the control cost", function () {
            ...
            expect(vpp.warnings[0]).toHaveProperty("message", "Slot active points are larger than the framework's control");
        ...
        it("should warn if the total allocated real points exceeds the pool size", function () {
            ...
            expect(vpp.warnings).toHaveLength(1);
            expect(vpp.warnings[0]).toHaveProperty("message", "More real points are allocated than fit in the framework's pool");
        ...
        it("should warn if more points are allocated to a slot than it's defined to have", function () {
            ...
            expect(vpp.warnings).toHaveLength(1);
            expect(vpp.warnings[0]).toHaveProperty("message", "This slot has more points allocated to it than it can use");
            expect(vpp.warnings[0]).toHaveProperty("scope", WarningScope.Slot);
