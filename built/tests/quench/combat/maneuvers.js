...
import { nextDialog, nextMessage, openCharacterSheet, } from "../helpers/sheets.js";
...
export function register(system, quench) {
    quench.registerBatch(`${system}.cucumber.combat.maneuvers`, function ({ describe, it, expect, before, afterEach, beforeEach }) {
        ...
        describe("Maneuvers", function () {
            ...
            describe("Standard Combat Maneuvers", function () {
                ...
                it("I should see the combat maneuvers in the maneuver table", function () {
                    const maneuverTable = this.sheet.find('.tab[data-tab="combat"] table.maneuvers');
            ...
            describe("Martial Maneuvers", function () {
                beforeEach(async function () {
                    this.sheet = await openCharacterSheet(this.character);
                    const addManeuver = this.sheet.find('a.item-create[data-type="maneuver"]');
                ...
                it("a new Martial Maneuver should appear", function () {
                    expect(Object.values(this.character.itemTypes.maneuver)).to.have.lengthOf(1);
                });
        ...
        describe("Rolling Maneuvers", function () {
            ...
            it("should have a labelled modifier field for more complex maneuvers", async function () {
                ...
                const dialog = await dialogP;
                try {
                    expect(dialog.element.find('input[name="ocv"]').val()).to.equal("10");
                    expectTextContent(dialog.element
                        .find('input[name="maneuver-modifier"]')
                        .prev()
                        .get(0)).to.equal("-v/10 where v is my velocity in meters");
                }
                finally {
                    await dialog.close();
                }
        ...
        describe("Maneuver ActiveEffects", function () {
            ...
            describe("Non-attack maneuvers", function () {
                ...
                it("activating them should send a message to chat", async function () {
                    ...
                    const brace = sheet.find('a.activate-maneuver[data-maneuver-name="Brace"]');
                ...
                it("Dodge should temporarily increase DCV to 12", async function () {
                    ...
                    await waitOneMoment();
                    expect(this.character.system.characteristics.dcv.total).to.equal(12);
                });
            ...
            describe("Ending maneuver effects", function () {
                beforeEach("given I have a temporary DCV penalty from the Shove maneuver", async function () {
                    await build.at(this).character().build();
                    await build.combat(this, [this.character]);
                    await this.combat.startCombat();
                    await waitOneMoment();
                    await this.character.activateManeuver(this.maneuvers.get("Shove"));
                });
                ...
                it("if I activate another maneuver, the Shove effect should end", async function () {
                    await this.character.activateManeuver(this.maneuvers.get("Haymaker"));
                    expect(this.character.effects.contents).to.have.lengthOf(1);
                    expect(this.character.effects.contents[0]).to.have.property("label", "Haymaker");
                });
    ...
    quench.registerBatch(`${system}.combat.maneuvers`, function ({ describe, it, expect }) {
