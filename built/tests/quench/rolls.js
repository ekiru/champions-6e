...
import { findDamageRollForAttack, nextDialog, nextMessage, openCharacterSheet, } from "./helpers/sheets.js";
/**
    ...
    class FakeDice extends Roll {
        async evaluate(options) {
            ...
            if (result.dice.length !== dice.length) {
                throw new Error(`dice lengths don't match for FakeDice: expected ${dice.length} but got ${result.dice.length}`);
            }
            dice.forEach((fakeDie, i) => {
                const realDie = result.dice[i];
                if (realDie.results.length !== fakeDie.length) {
                    throw new Error(`DiceTerm ${i} with unexpected length in FakeDice: expected ${fakeDie.result.length} but got ${realDie.results.length}`);
                }
...
export function register(system, quench) {
    quench.registerBatch(`${system}.rolls`, function ({ describe, it, expect, before, after, beforeEach, afterEach }) {
        // make ChatMessage a bit happier with our fake rolls
        before(function () {
            CONFIG.Dice.rolls.push(fakeRoller(11), fakeDice([]));
        });
        after(function () {
            const index = CONFIG.Dice.rolls.findIndex((cls) => cls.name === fakeRoller(11).name);
        ...
        describe("Attack rolls", function () {
            describe("With a known DCV", function () {
                ...
                it("should handle stringified number parameters gracefully", async function () {
                    const Roll = fakeRoller(4);
                    result = await rolls.performAttackRollWithKnownDcv("8", String(dcv), { Roll });
        ...
        describe("Normal damage rolls", function () {
            ...
            describe("chat messages", function () {
                ...
                beforeEach(async function () {
                    result = await rolls.performNormalDamageRoll(new Damage(7, 5, 0.5), { actor, label });
                });
        ...
        describe("Killing damage rolls", function () {
            ...
            it("should count the damage for half dice correctly", async function () {
                ...
                result = await rolls.performKillingDamageRoll(new Damage(3, 15, 0.5), { Roll });
            ...
            it("should count the damage for Xd6-1 correctly", async function () {
                const Roll = fakeDice([[1, 3, 6], [3]]);
                result = await rolls.performKillingDamageRoll(new Damage(3, 22.5, -1), {
                    Roll,
                });
                expect(result.body).to.equal(9);
            ...
            describe("chat messages", function () {
                ...
                beforeEach(async function () {
                    result = await rolls.performKillingDamageRoll(new Damage(3, 15, 0.5), {
                        actor,
                        label,
                    });
                });
    ...
    quench.registerBatch(`${system}.cucumber.rolls.advanced`, function ({ describe, it, expect, afterEach, beforeEach }) {
        describe("Advanced Dice Roller", function () {
            ...
            describe("Rolling a half-die", function () {
                ...
                it("when I click the damage button it should prepopulate the dice fields", async function () {
                    const damageRoll = await findDamageRollForAttack(this.character, "Lightning Bolt");
                    const dialogP = nextDialog();
                    damageRoll.click();
                    const dialog = await dialogP;
                    try {
                        expect(dialog.element.find("input[name='dice']").val()).to.equal("2");
            ...
            describe("Rolling dice +1", function () {
                ...
                it("when I enter 1d6+1 on the damage roll dialog, it should roll 1d6+1", async function () {
                    ...
                    diceField.val("1");
                    const d6PlusOne = dialog.element.find("select[name='diceSuffix'] > option:contains('d6+1')");
