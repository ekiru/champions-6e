...
import { findDamageRollForAttack, nextDialog, nextMessage, openItemSheet, openCharacterSheet, } from "./helpers/sheets.js";
...
export function register(system, quench) {
    ...
    quench.registerBatch(`${system}.combat.class`, function ({ describe, it, expect, beforeEach, afterEach }) {
    ...
    quench.registerBatch(`${system}.cucumber.combat.initiative`, function ({ describe, it, expect, before, after }) {
        ...
        describe("Initiative order", function () {
            ...
            after(async function () {
                await Promise.allSettled([dean, millie, norm].map((char) => char.delete()));
            });
            let combat;
            ...
            describe("Tie breaking", function () {
                it("should be performed automatically", async function () {
                    ...
                    for (const combatant of combat.combatants.contents) {
                        ...
                        expect(combatant.initiative, `Initiative was not rolled for ${combatant.name}`).to.exist;
                    }
                });
            });
            describe("The combat order table", function () {
                ...
                it("should show the dexes (and / tiebreaks) in the left column", function () {
                    const dexes = tracker.find("tbody > tr:not(.segment-header) > td:first-child");
                ...
                it("should show all the phases for this turn", function () {
                    ...
                    expectTextContent(phases.get(1).children[nameCol]).to.equal(millie.name);
                    expectTextContent(phases.get(2).children[nameCol]).to.equal(dean.name);
                    expectTextContent(phases.get(3).children[nameCol]).to.equal(norm.name);
                });
                it("should highlight Millie's phase on segment 12", function () {
                    expect(tracker.find("tr.active"), "only one current phase should exist").to.have.lengthOf(1);
                    const segment12 = $(tracker.find("tbody").get(0));
                    const currentPhase = segment12.find("tr.active");
                    expect(currentPhase, "current phase should be in segment 12").to.have.lengthOf(1);
                    expectTextContent(currentPhase.children().get(2)).to.equal(millie.name);
                });
            });
        });
    }, { displayName: `${system}: Initiative Order` });
    quench.registerBatch(`${system}.cucumber.combat.initiative.changes`, function ({ describe, it, expect, beforeEach, afterEach }) {
        describe("Changes to initiative order", function () {
            ...
            describe("Changing DEX in the middle of a segment", function () {
                ...
                it("Hare lowering her DEX shouldn't change the initiative order this segment", async function () {
                    ...
                    await waitOneMoment();
                    expect(this.combat.turns.map((c) => c.actorId)).to.have.ordered.members([this.hare.id, this.tortoise.id]);
                });
            });
    ...
    quench.registerBatch(`${system}.cucumber.combat.damage-classes.adder`, function ({ describe, it, expect, beforeEach, afterEach }) {
        describe("Damage Class Adder", function () {
            ...
            describe("Haymaker", function () {
                ...
                it("when they haymaker a HTH attack for +4 DC, it should roll 8½d6", async function () {
                    const attack = await findDamageRollForAttack(this.character, "Basic HTH Attack");
            ...
            describe("Adding DCs to Drain", function () {
                beforeEach("given my character has a Drain with 6d6", async function () {
                    await build.character(this, {});
                ...
                it("when they add 3 DC to it, it should roll 7½ d6", async function () {
                    const attack = await findDamageRollForAttack(this.character, "Drain");
                    const dialogP = nextDialog();
                    attack.click();
                    const dialog = await dialogP;
            ...
            describe("Adding DCs to a HKA", function () {
                beforeEach("given my character has a HKA with 2d6", async function () {
                    await build.character(this, {});
            ...
            describe("Adding STR", function () {
                ...
                describe("to a 2d6 Killing Attack", function () {
                    beforeEach("given my character's STR is 30 and I have a 2d6 HKA", async function () {
                        await build.character(this, {
                            characteristics: {
                    ...
                    it("should do 4d6 when I add my STR", async function () {
                        const attack = await findDamageRollForAttack(this.character, "Powerful Bite");
                        const dialogP = nextDialog();
                        attack.click();
                        const dialog = await dialogP;
                        dialog.element.find('input[name="addStr"]').click();
