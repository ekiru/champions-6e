...
export function register(system, quench) {
    quench.registerBatch(`${system}.characters`, function ({ describe, it, expect }) {
        const expectCharacteristic = _expectCharacteristic(expect, (char) => char.value);
        const expectCharacteristicMax = _expectCharacteristic(expect, (char) => char.max);
        describe("A new character", async function () {
            ...
            it("should have current END/BODY/STUN equal to their max", function () {
                ...
                function expectValueEqualsMax(char) {
                    expect(character.system.characteristics[char].value, `${char.toUpperCase()} current should equal max`).to.equal(character.system.characteristics[char].max);
                }
            ...
            it("should have Everyperson skills", function () {
                 ...
                 */
                function expectBackgroundSkill(skill, name, backgroundType, targetNumber) {
                    expect(skill).to.exist;
                    expect(skill.name).to.equal(name);
                ...
                return new Promise((resolve, reject) => {
                    setTimeout(function () {
                        try {
                            ...
                            expectBackgroundSkill(skills[5], "Area Knowledge: [home region]", "knowledge", 8);
                            expectSkill(skills[6], "Language: [native language] (completely fluent, literate)", 0);
                            expectCharBasedFamiliarity(skills[7], "Paramedics", "int");
                            expectCharBasedFamiliarity(skills[8], "Persuasion", "pre");
                            expectBackgroundSkill(skills[9], "[job or primary hobby]", "professional", 11);
                            expectCharBasedFamiliarity(skills[10], "Shadowing", "int");
                            expectCharBasedFamiliarity(skills[11], "Stealth", "dex");
                            expectSkill(skills[12], "TF: Small Motorized Ground Vehicles", 0);
                            resolve();
                        }
                        catch (e) {
                            reject(e);
        ...
        describe("Characteristic derived data", function () {
            ...
            it("should have 4d6 HTH damage and 6d6 PRE attack with 20 STR and 30 PRE", async function () {
                ...
                expect(character.system.characteristics.pre.presenceAttackDice).to.equal(6);
            });
        });
        describe("Conversion to a Character object", function () {
            ...
            it("should have the characteristics of the character", function () {
                expect(this.character.asCharacter.characteristic(OCV)).to.exist.and.have.property("value", 8);
            });
        });
    }, {
        displayName: `${system}: Test Character model`,
    });
    quench.registerBatch(`${system}.cucumber.characters.modifier_boxes`, function ({ describe, it, expect, afterEach }) {
        describe("Modifier boxes", function () {
            ...
            describe("Calculating characteristic rolls", function () {
                it("should be based on the total", async function () {
                    ...
                    expect(character.system.characteristics.str.targetNumber).to.equal(12);
                });
            });
            ...
            describe("Calculating PRE Attack dice", function () {
                it("should be based on the total PRE", async function () {
                    ...
                    expect(character.system.characteristics.pre.presenceAttackDice).to.equal(5);
                });
            });
    ...
    quench.registerBatch(`${system}.cucumber.characters.modifier_boxes.ui`, function ({ describe, it, expect, before, after }) {
        describe("the character sheet", function () {
            ...
            before(async function () {
                ...
                row = sheet.find('table.main-characteristics > tbody > tr:contains("STR")');
    ...
    quench.registerBatch(`${system}.cucumber.characters.hap`, function ({ describe, it, expect, before, after }) {
        let character;
        let sheet;
        before(async function () {
            character = await Actor.create({
        ...
        describe("Rolling HAP at the start of a session", function () {
            it("my HAP should equal the results of the roll", async function () {
                ...
                expect(sheet.find(".hap input").val()).to.equal(rollMessage.rolls[0].result);
            });
    ...
    quench.registerBatch(`${system}.cucumber.characters.derived_attributes`, function ({ describe, it, expect, beforeEach, afterEach }) {
        describe("Showing characteristic TNs", function () {
            ...
            it("I should see that my EGO roll has a target number of 10", async function () {
                const row = sheet.find('table.main-characteristics > tbody > tr:contains("EGO")');
                expect(row.length).to.equal(1);
                expect(row.children().get(4).textContent.trim()).to.match(/^Roll\s+10-$/);
            });
        });
        ...
        describe("Lifting weight", function () {
            ...
            it("should be calculated correctly based on STR", async function () {
                ...
                for (const [str, weight] of examples) {
                    ...
                    expect(character.system.characteristics.str.liftingWeight.value).to.equal(Number(value));
                    expect(character.system.characteristics.str.liftingWeight.unit).to.equal(unit);
                }
            });
        });
